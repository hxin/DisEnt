...
my $sth = $dbh->prepare("select distinct variation_id from human_variation;");
$sth->execute();
my @vars;
...
foreach my $var(@vars){
	##overlap
	$sth=$dbh->prepare("
(SELECT distinct variation_id,ensembl_id, 'o' as position 
...
where variation_id=?)
union all
(select variation_id,ensembl_id,'d' as position 
from human_gene as g,human_variation as v
where g.chromosome_name=v.chromosome_name and g.start_position>v.position and variation_id=?
order By ABS(g.start_position-v.position) Asc limit 1)
union all
(select variation_id,ensembl_id,'u' as position 
from human_gene as g,human_variation as v
where g.chromosome_name=v.chromosome_name and g.end_position<v.position and variation_id=? 
