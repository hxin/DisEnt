...
#		exit if $line_count>100;
		...
		if($start){
			if($line =~ m/^id:\s(.+)/){
				...
				$return{$current_id}->{'is_obsolete'}=0;
	        		next READLINE;       
			}elsif($line =~ m/^name:\s(.+)/){
...
#	$hash_ref->{'def'}='\N' if !defined($hash_ref->{'def'});
#	$hash_ref->{'comment'}='\N' if !defined($hash_ref->{'comment'});
#	print $do_acc."\t".$hash_ref->{'name'}."\t".$hash_ref->{'def'}."\t".$hash_ref->{'comment'}."\n";
#}			
#}
...
while (my ($do_acc, $hash_ref) = each(%return)){
	$hash_ref->{'def'}='\N' if !defined($hash_ref->{'def'});
	$hash_ref->{'comment'}='\N' if !defined($hash_ref->{'comment'});
	print $do_acc."\t".$hash_ref->{'name'}."\t".$hash_ref->{'def'}."\t".$hash_ref->{'comment'}."\t".$hash_ref->{'is_obsolete'}."\n";
}
