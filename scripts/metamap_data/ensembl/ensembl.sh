...
if [ $USECACHE = 'n' ]; then
echo 'loading phenotypes from db...'$(date +"%T")
echo 'loading phenotypes from db...'$(date +"%T") >>$LOG
perl $BASEDIR/get_dis.pl $DB $HOST $USER $PSW > $BASEDIR/tmp/pts
...
split -n l/$METAMAP_CHUNK_NUMBER -a 3 -d $BASEDIR/tmp/pts pts_chunk_
mv `pwd`/pts_chunk_* $BASEDIR/tmp/chunks/
...
cp $BASEDIR/tmp/chunks/all $BASEDIR/tmp/tmp_ensembl_mmtx_0
...
perl $BASEDIR/ensembl.pl $DB $HOST $USER $PSW $BASEDIR/tmp/tmp_ensembl_mmtx_1 >$BASEDIR/tmp/MetaMap_ensembl2do_raw
fi
...
mysqlimport -h $HOST -u $USER -p$PSW --delete -L $DB $BASEDIR/tmp/MetaMap_ensembl2do_raw
