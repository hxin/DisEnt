...
my $result_ref={};
my $result_eq_ref={};
my $table='MetaMap_omim2do_raw';
my $sth = $dbh->prepare("
select disorder_mim_acc,do_acc,score from $table");
$sth->execute();
...
foreach my $key(keys %$result_ref){
	foreach(@{$result_ref->{$key}}){
		my $d=$_;
		print $key."\t".oid2des($key)."\t".$d."\t".hdoid2des($d)."\n";
	}
...
sub oid2des{
	my $id=shift @_;
	my $sth = $dbh->prepare("select description from OMIM_disease2gene where disorder_mim_acc=?; ");
	$sth->execute($id);
