...
if [ $USECACHE = 'n' ]; then
	echo "[$(date +"%T %D")] Loading phenotypes from db..."
	...
	cp $CHUNK/all $CHUNK/../NCBO_ensembl2do_raw
fi
...
perl $BASEDIR/getpts.pl $DB $HOST $USER $PSW > $BASEDIR/tmp/pts
...
split -n l/$NCBO_CHUNK_NUMBER -a 3 -d $BASEDIR/tmp/pts pts_chunk_
...
cp $BASEDIR/tmp/chunks/all $BASEDIR/tmp/NCBO_ensembl2do_raw
fi
