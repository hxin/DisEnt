...
select t1.entrez_id,t1.ensembl_id,t2.pmid,t2.do_acc as disease_id,0 as `did_type`,t2.rif from human_gene as t1 right join GeneRIF_dga as t2
on t1.entrez_id=t2.entrez_id;
ALTER TABLE `GeneRIF_d2g` ADD INDEX (`entrez_id`);
ALTER TABLE `GeneRIF_d2g` ADD INDEX (`ensembl_id`);
ALTER TABLE `GeneRIF_d2g` ADD INDEX (`pmid`);
ALTER TABLE `GeneRIF_d2g` ADD INDEX  (`did_type`);
ALTER TABLE `GeneRIF_d2g` ADD INDEX  (`disease_id`);
