...
my $sth = $dbh->prepare("SELECT distinct do_acc FROM GeneRIF_disease2gene_dga;");
$sth->execute();
my @ids;
...
foreach my $id(@ids){
	...
	if(my @row = $sth->fetchrow_array){
		$sth = $dbh->prepare("UPDATE GeneRIF_disease2gene_dga SET do_acc=?
				WHERE do_acc=?");
		$sth->execute($row[0],$id);
	}
}

$sth = $dbh->prepare("SELECT distinct entrez_id FROM GeneRIF_disease2gene_dga;");
$sth->execute();
my @genes;
while ( my @row = $sth->fetchrow_array ) {
	push(@genes,$row[0]);
...
foreach my $id(@genes){
	...
	$sth->execute($id);
	if(my @row = $sth->fetchrow_array){
		$sth = $dbh->prepare("UPDATE GeneRIF_disease2gene_dga SET entrez_id=?
				WHERE entrez_id=?");
		$sth->execute($row[0],$id);
	}
}
